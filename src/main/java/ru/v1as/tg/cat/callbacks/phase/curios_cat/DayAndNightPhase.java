...
public class DayAndNightPhase extends AbstractCuriosCatPhase {
    ...
    private void dayStory(ChooseContext ctx) {
        /*--- полная история, как если всё видно:
            - Любопытный Кот ведёт вас за собой
        - следуя за Котом, вы попадаете в парк
        - Кот движется к непримечательному кусту и скрывается в нём
        - через мгновение Кот выскакивает оттуда, как ошпаренный
        - следом за ним вылетает ещё один кот
        - оба кота кружат вокруг друг друга, готовятся к драке - коты завывают, горбят спины
        - что дальше?
            > распугать котов > ваш окрик разгоняет котов, вы не успели их сосчитать
            > обойти > вы осторожно обходите серьёзно настроенных котов => два кота*/
        messages(
                "Следуя за котом, вы попадаете в парк.",
